...
sealed class ResultData<out T> {
    ...
    data class Logout<out T>(val data: T? = null): ResultData<T>()
    data class Loading(val nothing: Nothing? = null): ResultData<Nothing>()
    data class Failed(val status: String? = null, val message: String? = null, val title: String? = null,val specialError: Boolean =  false): ResultData<Nothing>()
